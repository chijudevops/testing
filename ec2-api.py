...
def create_configs_check(instances, templateHost, noClean):
    for instance in instances:
...
def create_configs_check(instances, templateHost, templateCheck, noClean):
    for instance in instances:
        if not os.path.exists(icinga2ConfigDir + "conf.d/hosts/" + instance.id):
            os.makedirs(icinga2ConfigDir + "conf.d/hosts/" + instance.id)
            writeTemplate(templateHost, instance.id, instance.id, instance)
...
if hasattr(args, 'command') and args.command == 'clean':
    ...
    if args.templateCheck:
        ...
        create_configs(instances, templateHost, args.noClean)
    else:
        templateHost = args.templateHost.read()
        create_configs_check(instances, templateHost, templateCheck, args.noClean)
